						...
						<table class="table table-bordered">
							<thead>
								<tr>
									...
									<th class="product-quantity">Placed On</th>
									<th class="product-total">Assigned Employee ID</th>
							...
							<tbody>
								...
								<c:set var="orderstatuspending" value="PENDING" />
								<c:set var="orderstatuscancelled" value="CANCELLED" />
								<c:set var="orderstatusdelivered" value="DELIVERED" />
								<c:forEach items="${orders }" var="order">
									<tr>
										<td><a href="/admin/vieworderinfo/${order.orderid}">${order.orderid }</a></td>
										<td><a href="/admin/viewcustinfobyid/${order.custid }">${order.custid }</a></td>
										<td>${order.totalamount }</td>
										<td>${order.orderdatetime }</td>

										<c:if test="${order.assignedemployeestatus == status and order.orderstatus == orderstatuspending}">
											<td class="bg-light">Not assigned!
											<hr><a href = "/admin/assignemployee/${order.orderid }">Click here to assign</a></td>
										</c:if>
										<c:if test="${order.assignedemployeestatus != status and order.orderstatus == orderstatuspending}">
											<td class="bg-light"><a
												href="/admin/viewemployeeinfobyid/${order.assignedemployeeid }">${order.assignedemployeeid }</a>
												<hr><a href = "/admin/assignemployee/${order.orderid }">Click here to assign another employee.</a></td></td>
										</c:if>
										<c:if test="${order.orderstatus == orderstatuscancelled}">
											<td class="bg-light">Ordered Cancelled.</td>
										</c:if>
										<c:if test="${order.orderstatus == orderstatusdelivered}">
											<td class="bg-light"><a href = "/admin/viewemployeeinfobyid/${order.assignedemployeeid}">${order.assignedemployeeid }</a>
										...
										<c:if test="${order.orderstatus == orderstatuspending}">
